...
app.get("/api/boxoffice", async (req, res) => {
  try {
    ...
    console.log(`[KOBIS API Proxy] GET /api/boxoffice?date=${date} fetched successfully.`);
    if (data.faultInfo) {
      console.error("[KOBIS API Proxy] Error returned from Kobis server:", data.faultInfo);
    }
    res.json(data);
  } catch (error: any) {
    console.error("Error fetching box office data in proxy: ", error);
    res.status(500).json({ error: "Failed to fetch box office data via proxy", details: error.message });
  }
});
...
app.get("/api/movie", async (req, res) => {
  try {
    ...
    console.error("Error fetching movie details in proxy: ", error);
    res.status(500).json({ error: "Failed to fetch movie details via proxy", details: error.message });
  }
});

// 3. AI Movie Review Generation Route (Gemini API)
  ...
  try {
    ...
    if (!geminiKey) {
      return res.status(400).json({
        error: "GEMINI_API_KEY가 설정되지 않았습니다. Vercel 환경 변수 혹은 AI Studio의 Settings > Secrets 패널에서 API 키를 설정해주세요."
      });
    }
...
`;
    ...
    console.error("Error generating AI review in proxy: ", error);
